...
void reset_canvas(int, int);                                  // キャンバスをリセット
char *extract_option_name(char *, int *, char *);             // ファイル名に指定された指定された画像の名前を返す
void scroll_panel(int, int, int, int, unsigned char *);
...
int main(int argc, char **argv)
{
    ...
    int offset = 16;
    scroll_image = (unsigned char *)calloc(led_width * led_height * 3 * 2, sizeof(unsigned char) + offset * led_width * 3);
    if (!scroll_image)
    {
        exit(11);
    }
    ...
    while (1)
    {
        for (int i = 0; i < file_num; i++)
        {
            ...
            scroll_panel(led_width - image_x, led_height - image_y, image_x, image_y, pixel); // データをキャンバスに反映
...
void scroll_panel(int start_x, int stary_y, int data_width, int data_height, unsigned char *pixel)
{
    ...
    memcpy(&scroll_image[0], &scroll_image[panel_x * +panel_y * 3 + offset * panel_x * 3], panel_x * +panel_y * 3 * sizeof(unsigned char)); // スクロール先 領域にあった画像をスクロール元領域に移動

    for (int y = 0; y < data_height; y++) // pixelをスクロール先領域に格納
    {
        for (int x = 0; x < data_width; x++)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                scroll_image[i + n + panel_x * +panel_y * 3 + offset * panel_x * 3] = pixel[i + n];
            }
        }
    }
    ...
    for (int i = 0; i < panel_y + offset; i++) // スクロール処理
    {
        for (int y = 0; y < data_height; y++)
        {
            ...
            {
                n = (y * data_width + x) * 3;
                led_canvas_set_pixel(offscreen_canvas, start_x + x, stary_y + y, scroll_image[i * panel_x * 3 + n], scroll_image[i * panel_x * 3 + n + 1], scroll_image[i * panel_x * 3 + n + 2]); // キャンバスの座標x,yの色を設定
            }
        }
        offscreen_canvas = led_matrix_swap_on_vsync(matrix_options, offscreen_canvas); // キャンバスをパネルに反映
        delay(40);
