// g++cmd:g++ -o led_07 led_07.c -Wall -I /home/metoro/rpi-rgb-led-matrix/include -L /home/metoro/rpi-rgb-led-matrix/lib -lrgbmatrix -I /home/metoro/stb -lwiringPi -g -O0
...
void set_image(int, int, int, int, unsigned char *);
void scroll_up_panel(int);
...
unsigned char *image_buf;            // 画像バッファ
...
int main(int argc, char **argv)
{
    ...
    image_buf = (unsigned char *)calloc(sizeof(unsigned char), led_width * led_height * 3 * 3 + offset * led_width * 3 * 2);
    if (image_buf == NULL)
    {
        exit(1);
    }
    ...
    while (1)
    {
        ...
        for (int i = 0; i < file_num; i++)
        {
            ...
            memcpy(&image_buf[(led_height + offset) * led_width * 3], &image_buf[(led_height * 2 + offset * 2) * led_width * 3], led_width * led_height * 3); // スクロール先領域の幕をスクロール元領域へ移動
            memset(&image_buf[(led_height * 2 + offset * 2) * led_width * 3], 0, led_width * led_height * 3);
            memset(&image_buf[0], 0, led_width * led_height * 3); // スクロール先領域をリセット

            int image_offset_x = led_width - image_x;
            int image_offset_y = led_height - image_y;

            set_image(image_offset_x, image_offset_y, image_x, image_y, pixel);

            if (strstr(file_list[i], "_S") != NULL) // _S"を含む(スクロール指定)な
            {
                scroll_up_panel(50); // スクロール
                sleep(wait_time);
            }
            else
            {
            ...
            for (int j = 0; j < option_num; j++)
            {
                ...
                memcpy(&image_buf[0], &image_buf[led_width * led_height * 3 + offset * led_width * 3], led_width * led_height * 3); // スクロール先領域の幕をスクロール元領域へ移動
                memset(&image_buf[led_width * led_height * 3 + offset * led_width * 3], 0, led_width * led_height * 3);             // スクロール先領域をリセット
                ...
                if (strstr(file_list[i], "_S_.") != NULL) // _S"を含む(スクロール指定)な
                {
                    scroll_up_panel(50); // スクロール
                    sleep(wait_time);
                }
...
void scroll_up_panel(int speed)
{
    for (int i = 0; i <= led_height + offset; i++) // スクロール処理
    {
        for (int y = 0; y < led_height; y++)
        {
            for (int x = 0; x < led_width; x++)
            {
                int n = (y * led_width + x) * 3;
                led_canvas_set_pixel(offscreen_canvas, x, y, image_buf[(i + led_height + offset) * led_width * 3 + n], image_buf[(i + led_height + offset) * led_width * 3 + n + 1], image_buf[(i + led_height + offset) * led_width * 3 + n + 2]); // キャンバスの座標x,yの色を設定
            }
        }
...
void change_panel(void)
{
    for (int y = 0; y < led_height; y++)
    {
        ...
        {
            int n = (y * led_width + x) * 3;
            int i = led_height * 2 + offset * 2;
            led_canvas_set_pixel(offscreen_canvas, x, y, image_buf[i * led_width * 3 + n], image_buf[i * led_width * 3 + n + 1], image_buf[i * led_width * 3 + n + 2]); // キャンバスの座標x,yの色を設定
        }
    }
    offscreen_canvas = led_matrix_swap_on_vsync(matrix_options, offscreen_canvas); // キャンバスをパネルに反映
}

void set_image(int image_offset_x, int image_offset_y, int image_x, int image_y, unsigned char *pixel)
{
    for (int y = 0; y < image_y; y++)
    {
        for (int x = 0; x < image_x; x++)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                image_buf[led_width * (led_height * 2 + offset * 2) * 3 + ((image_offset_y + y) * led_width + (image_offset_x + x)) * 3 + i] = pixel[n + i];
            }
        }
    }
}